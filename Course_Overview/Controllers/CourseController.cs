...
namespace Course_Overview.Controllers
{
	public class CourseController : Controller
	{
		...
		[HttpPost]
		public async Task<IActionResult> RegisterCourse( CourseRegistration registration)
		{
			// Lấy StudentId từ session
			var studentId = HttpContext.Session.GetInt32("StudentID");
			if (studentId == null)
			{
				return BadRequest("Student is not logged in.");
			}

			// Kiểm tra xem CourseId có tồn tại trong bảng Courses không
			var courseExists = await _dbContext.Courses.AnyAsync(c => c.CourseID == registration.CourseID);
			if (!courseExists)
			{
				return BadRequest("Course does not exist.");
			}

			registration.StudentID = (int)studentId; // Gán StudentId
			registration.RegistrationDate = DateTime.Now;
			registration.Status = "Pending"; // Trạng thái ban đầu là Pending
